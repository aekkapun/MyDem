...
class Vehicle extends \yii\db\ActiveRecord {
    ...
    const UPLOAD_FOLDER = 'vehicle_file';
    ...
    public function rules() {
        return [
            ...
            [['CORCDE', 'ENG_TYP', 'FUEL_TYP', 'GPS', 'VHCVAL'], 'string', 'max' => 20],
    ...
    public function getDoc($ref, $event_name) {
        $uploadFiles = PermitCar::find()->where(['ref' => $ref])->all();
        $preview = [];
        foreach ($uploadFiles as $file) {
            $preview[] = [
                'url' => self::getUploadUrl(true) . $ref . '/' . $file->real_filename,
    ...
    public function listDownloadFiles($type) {
        $docs_file = '';
        if (in_array($type, ['ATTACH_FILE', 'ATTACH_FILE'])) {
            $data = $type === 'ATTACH_FILE' ? $this->ATTACH_FILE : $this->ATTACH_FILE;
            ...
            foreach ($files as $key => $value) {
                if ($type == 'file') {
                    ...
                    $initial[] = [
                        ...
                        'width' => '120px',
                        'url' => Url::to([''
                            . '/car/vehicle-register/deletefile', 'id' => $this->ID, 'fileName' => $key, 'field' => $field]),
                        'key' => $key
                    ];
                } else {
