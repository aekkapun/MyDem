...
// เพิ่มรายการคำขอ
    public function actionCreate() {
        ...
        if ($model->load(Yii::$app->request->post()) && $modelRequest->load(Yii::$app->request->post()) && Model::validateMultiple([$model, $modelRequest])) {
            ...
            if ($model->save()) {
                ...
                $modelRequest->OWNER_ID = $model->ID;
                $modelRequest->save();
            }
    ...
    private function getInitialPreview($ref) {
        ...
        foreach ($datas as $key => $value) {
            array_push($initialPreview, $this->getTemplatePreviewAjax($value));
    ...
    private function getTemplatePreview(Uploads $model) {
        $filePath = PermitCar::getUploadUrl() . $model->REQ . '/thumbnail/' . $model->real_filename;
